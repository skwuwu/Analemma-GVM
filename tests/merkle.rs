...
async fn merkle_event_hash_embedded_in_wal() {
    ...
    let stored: GVMEvent = serde_json::from_str(event_lines[0]).unwrap();
    assert!(
    ...
    assert_eq!(
        stored.event_hash.as_ref().unwrap(),
        &recomputed,
...
async fn merkle_batch_root_recomputable() {
    ...
    for line in content.lines() {
        if line.contains("\"merkle_root\"") {
            ...
            let record: MerkleBatchRecord = serde_json::from_str(line).unwrap();
            ...
            current_batch_hashes.push(event.event_hash.unwrap());
        }
    }
}
...
async fn merkle_inter_batch_chain() {
    ...
    let ledger = Ledger::with_config(&wal_path, "", "", config).await.unwrap();
...
async fn merkle_wal_verification_detects_tampered_event() {
    ...
    for line in lines.iter_mut() {
        if !line.contains("\"merkle_root\"") {
            if let Ok(mut event) = serde_json::from_str::<GVMEvent>(line) {
                ...
                *line = serde_json::to_string(&event).unwrap();
                break;
            }
        }
    }
    ...
    let first_event_line = tampered
        ...
        .unwrap();
    let tampered_event: GVMEvent = serde_json::from_str(first_event_line).unwrap();
    let recomputed = merkle::compute_event_hash(&tampered_event);
    assert_ne!(
        tampered_event.event_hash.as_ref().unwrap(),
        &recomputed,
    ...
    for line in lines.iter_mut() {
        if line.contains("\"merkle_root\"") {
            ...
            if batch_count == 2 {
                ...
                let mut record: MerkleBatchRecord = serde_json::from_str(line).unwrap();
...
async fn merkle_proof_proves_event_in_batch() {
    ...
    for line in content.lines() {
        if line.contains("\"merkle_root\"") {
            ...
            batch_event_hashes.push(event.event_hash.unwrap());
        }
    }
