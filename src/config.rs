...
pub struct ProxyConfig {
    ...
    /// Ed25519 anchor signing configuration (optional).
    /// When configured and `enabled = true`, every WAL anchor record
    /// is signed so an external auditor can verify the chain
    /// originated from a key the operator declared in advance.
    /// `None` (or `enabled = false`) means anchors are produced
    /// without signatures (NoopSigner) — the WAL Merkle chain still
    /// detects internal tamper, but cannot prove origin.
    pub anchor: Option<AnchorSigningConfig>,
...
/// Ed25519 anchor signing configuration.
///
/// ```toml
/// [anchor]
/// enabled = true
...
/// ```
///
/// **Operational contract.** When `enabled = true`, the proxy
/// REFUSES to start unless `key_path` exists, parses, and contains
/// a valid Ed25519 secret. This is intentional: the alternative is
/// silently downgrading to NoopSigner, which would mean unsigned
/// anchors land in the WAL exactly when an operator thought they
/// were getting signed audit. Fail-close.
///
/// **Key file format.** TOML with at minimum:
/// ```toml
/// key_id = "gvm-prod-1"
/// algorithm = "ed25519"
/// secret_hex = "<64-character hex of 32 raw bytes>"
/// created_at = "2026-05-07T00:00:00Z"
/// ```
/// Generated by `gvm anchor keygen` (which also writes the
/// matching `.pub` file with `public_hex` for distribution).
///
/// **Why `key_id` lives in the file, not in this config.** The
/// keys file is the single source of truth: rotating to a new key
/// is "swap the file, restart the proxy". Embedding `key_id` in a
/// separate config field invites drift.
#[derive(Deserialize, Clone, Debug)]
pub struct AnchorSigningConfig {
    /// When `true`, every anchor record is Ed25519-signed by the
    /// key at `key_path`. Default `false` (anchors unsigned —
    /// matching pre-Strategic-5 behavior on existing deployments).
    #[serde(default)]
    pub enabled: bool,
    /// Path to the TOML key file produced by `gvm anchor keygen`.
    /// Mode 0600, owned by the proxy uid. Required when
    /// `enabled = true`.
    pub key_path: Option<String>,
}

/// On-disk representation of an anchor secret key. Produced by
/// `gvm anchor keygen` and consumed at proxy startup. The
/// matching public key is written to `<key_path>.pub` with
/// `public_hex` instead of `secret_hex`; auditors receive only
/// the public file.
#[derive(Deserialize, Debug)]
pub struct AnchorKeyFile {
    /// Operator-assigned label. Used as `key_id` in every anchor
    /// signature and as the lookup key in a verifier registry.
    /// Auditors map `key_id` → `VerifyingKey` to know which public
    /// key validates each signature.
    pub key_id: String,
    /// Algorithm tag. Currently always `"ed25519"`. Reserved for
    /// future HSM / TSA / multi-algorithm support — verifier
    /// rejects unknown values.
    pub algorithm: String,
    /// Raw secret as 64 hex characters (32 bytes). Held in memory
    /// only long enough to construct the `SigningKey`, which has
    /// `ZeroizeOnDrop`.
    pub secret_hex: String,
    /// Creation timestamp (RFC 3339). Informational; the verifier
    /// does not enforce validity windows. Useful in audit reports.
    #[serde(default)]
    pub created_at: Option<String>,
}

impl AnchorKeyFile {
    /// Load and validate a key file. Returns the parsed struct on
    /// success. Errors are intentionally specific so an operator
    /// can diagnose without dumping secret bytes into a log.
    pub fn load(path: &Path) -> Result<Self> {
        let raw = std::fs::read_to_string(path)
            .with_context(|| format!("Failed to read anchor key file: {}", path.display()))?;
        let parsed: AnchorKeyFile = toml::from_str(&raw).with_context(|| {
            format!(
                "Failed to parse anchor key file as TOML: {}",
                path.display()
            )
        })?;
        ...
        if parsed.algorithm != "ed25519" {
            anyhow::bail!(
                "anchor key file algorithm must be \"ed25519\", got \"{}\"",
                parsed.algorithm
            );
        }
        if parsed.key_id.is_empty() {
            anyhow::bail!("anchor key file key_id is empty");
        }
        // 32 raw bytes → 64 hex chars. Validate length without
        // touching the secret material so a wrong-length file
        // produces a clear error before decode.
        if parsed.secret_hex.len() != 64 {
            anyhow::bail!(
                "anchor key file secret_hex must be 64 hex characters (32 raw bytes), got {}",
                parsed.secret_hex.len()
            );
        }
        // Hex parse just to surface "invalid hex" early; the full
        // 32-byte array is reconstructed by the caller.
        hex::decode(&parsed.secret_hex)
            .map_err(|_| anyhow::anyhow!("anchor key file secret_hex is not valid hex"))?;

        Ok(parsed)
    }

    /// Decode `secret_hex` into the 32-byte secret. Caller is
    /// responsible for treating the returned array as sensitive
    /// (typically passed straight into `SelfSignedSigner::from_secret`,
    /// which wraps it in a `ZeroizeOnDrop` `SigningKey`).
    pub fn secret_bytes(&self) -> Result<[u8; 32]> {
        let v = hex::decode(&self.secret_hex)
            .map_err(|_| anyhow::anyhow!("anchor key secret_hex decode failure"))?;
        let arr: [u8; 32] = v
            .try_into()
            .map_err(|_| anyhow::anyhow!("anchor key secret_hex did not decode to 32 bytes"))?;
        Ok(arr)
    }
}
