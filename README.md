...
- Policy evaluation ~1 µs on shared t3.medium (SRR pattern-match, per-arm O(1) — 68-rule production corpus); sub-µs on dedicated cores. Full hot-path replication (RwLock + Classification struct build) measures 1.24 µs. Cooperative-lease claim 1.35 µs at 1 000 active leases (O(1) reverse index landed 2026-07-05). Full number matrix and rationale in [docs/test-report.md § D.1.3](docs/test-report.md#d13-benchmark-integrity-audit--true-e2e-2026-07-11), [§ D.1.2](docs/test-report.md#d12-benchmark-refresh-2026-07-05--cooperative-lease-on--o1), and [§ D.1.1](docs/test-report.md#d11-benchmark-refresh-2026-06-19--post-v063-cooperative-lease-epic).
