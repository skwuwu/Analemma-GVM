...
> **Status: internal self-review — no external audit has been engaged.**
> This document is the snapshot we maintain so that an external
> reviewer could be onboarded quickly *if and when* an engagement is
> scheduled. Until then, every "the auditor would" / "we would hand
> over" sentence below is a forward-looking plan, not a description
> of work in progress.
>
> Companion to [security-model.md](../security-model.md) (public) and
> [GVM_CODE_STANDARDS.md](GVM_CODE_STANDARDS.md) (internal).
>
> Last updated 2026-05-07.

---

## 1. Scope (review surface)

### In scope (must be reviewed before v1)
...
- The `--contained` Docker mode — gated behind `cargo build --features contained`; default builds do not expose it.
- Any agent-side SDK code. The repository ships proxy + CLI only;
  governance is enforced at the proxy. There is no Python/Node SDK in
  this tree.
- The optional Anthropic/OpenAI client library code paths.
...
- Verify that documented security properties (fail-close, secret
  hygiene, JWT identity mandatory in production, anchor signing
  fail-close) are enforced by code, not just by convention.
...
### Top-priority attack scenarios

In rough order of impact-if-exploited:
...
- Anchor signing key, when `[anchor] enabled = true`, lives at the
  ...
  the public key registry to add the new public key).
...
they are explicit accepted risks that a reviewer should evaluate but
...
| L4 | LLM client library key handling | Out of scope | LLM client libraries need the key at construction. Use env var `ANTHROPIC_API_KEY`; proxy governs subsequent tool-API calls. |
...
| Fix | Commit | Test |
|-----|--------|------|
| Anchor signing wired up — runtime had a `SelfSignedSigner` impl + `with_config_and_signer` constructor but `main.rs` never called it, so production anchors landed unsigned. Added `[anchor]` config section, `gvm anchor keygen` CLI, fail-close on missing/malformed key file, doc + runtime aligned. | `72fa90b` | `crates/gvm-cli/src/anchor.rs` 4 unit tests (keygen round-trip, refuse-clobber, key_id validation); `tests/anchor_signing.rs` covers signer round-trip; EC2 E2E verified every anchor SelfSigned by `key_id=e2e-test-1` |
...
| Sandbox-peer identity for plain-HTTP-client agents | `3ec17bd` | `tests/api_handlers.rs::resolve_identity_from_peer_*` (3 cases) |
| IC-3 high-fanout & sweeper stress | `893c0e5` | `tests/stress.rs::ic3_*` (4 cases) — 1000 fan-out routing, 1500 dashmap, sweeper correctness, concurrent insert race |
...
| 12 cleanup gaps from prior internal audit (heartbeat, ip_forward, proxy.log rotation, EmergencyWAL cap, iptables substring, …) | `9fb036a` | `tests/cleanup_*.rs` (suite) |

---

## 6. Highest-value review focus (today)

If we engaged a reviewer tomorrow, these are the targets we would
ask them to start with — in priority order:
...
2. **`src/proxy/mod.rs::proxy_handler` step 0.** The sandbox-peer
...
3. **`crates/gvm-sandbox/src/network.rs::lookup_sandbox_id_by_ip`.**
   This trusted lookup is part of the identity trust path.
...
```bash
# Cross-platform unit + integration
...
sudo bash scripts/multi-agent-load.sh --agents 20 --requests 50

# Fuzz (time-boxed, optional)
...
Independent of any formal audit engagement, security findings can be
reported via:

- GitHub Security Advisory (preferred)
- Encrypted email to the security contact (separate channel; the
  GPG key is shared on request)

Severity scale: CVSS v3.1.

For each finding the response includes: triage class (true positive /
accepted risk / out of scope), fix ETA, and a backport plan if
applicable.

---

## 9. Engagement-readiness checklist

If and when an external engagement is scheduled, these are the
artifacts that would be handed to the reviewer at kickoff:

- [ ] Read access to this repo
- [ ] A pre-built `gvm-proxy` + `gvm` binary (release profile) for
      the target platform
...
This list is the operator's pre-engagement preparation, not a
record of any current handover.
