...
**Implementation status (2026-03)**: `gvm run --sandbox` (Linux-native) is production-ready, verified with 30-minute chaos stress tests (proxy kill, network partition, disk pressure — memory +2.4MB, FD stable, WAL verified). `gvm run --contained` (Docker) is implemented but experimental — unstable due to WSL2 network bridge issues, iptables unavailability in slim images, and `NET_ADMIN` capability constraints; stabilization planned for a future release. Containment is opt-in per launched process; workloads not started via `gvm run` remain in cooperative proxy mode. Sandbox launch is fail-fast gated on critical host prerequisites (`kernel.unprivileged_userns_clone=1`, `CAP_NET_ADMIN`, `ip`, `iptables`, `net.ipv4.ip_forward=1`). `gvm run` also performs proxy readiness checks before launch and auto-starts `gvm-proxy` only for localhost targets when the proxy is unavailable.
