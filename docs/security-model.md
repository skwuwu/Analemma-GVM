...
- **Time-bounded permission grants via IC-3 + orchestrator** — *planned (v0.7)*. When an external orchestrator needs to grant an agent additional permission for a bounded window, GVM does not introduce in-runtime JWT-scope matching (which would promote JWT verification to a hot-path cross-agent privilege boundary, see [removal rationale](internal/CHANGELOG.md#removed-from-this-roadmap-on-2026-05-11-architectural-reframing)). Instead, the orchestrator composes three primitives: (a) the existing IC-3 `RequireApproval` mechanism, which already expresses per-action human-in-the-loop grants and emits WAL events; (b) a planned WAL event stream subscription so the orchestrator receives `Pending` events without polling; (c) a planned granular SRR rule add/remove API with `expires_at` for inserting time-bounded `Allow` rules that auto-cull. The runtime stays a recorder + enforcer of declarative rules with no decision logic; the orchestrator decides scope and duration *outside* the runtime, where the OS-process / namespace boundary already provides structural enforcement. Same expressive power as JWT-scoped per-agent rules without any cross-agent boundary inside the runtime.
...
- *Within-rule median ratio* < 1.6× — for two requests that match the SAME URL pattern but take different sub-paths inside the matching rule (specifically: `POST /graphql` with `operationName=TransferFunds` versus `POST /graphql` with no `operationName` field, where the rule's payload check matches in one case and skips in the other). The URL/method **does not** disclose the sub-path here, so a measurable timing delta IS a side channel — but see the absolute-time analysis below.
- *Absolute cross-scenario delta* < 5,000 ns — the spread between the fastest and slowest scenario medians. Measured ≈1,900 ns on the reference hardware. This is the **operational** guarantee: it sits **~26× below** the loopback TCP RTT jitter floor (~50 µs) and **~260× below** the LAN jitter floor (~500 µs). A timing leak this small is not network-exploitable — a remote attacker would need ~10⁶ samples to average out jitter, and the per-agent `TokenBudget` (60-slot circular buffer over 1 hour) rate-limits long before that.

**Threat-model resolution**:

A timing oracle below the network jitter floor is exploitable only by an attacker with **cycle-accurate measurement** — i.e., one co-located on the same physical CPU with a shared cache. Per the [Assumption of Trust](#assumption-of-trust) section above, an attacker with that level of host access already has resources that subsume any timing-derived information (they can read the SRR config from disk, parse the WAL, sniff request bytes from `/proc/<pid>/net`). The within-rule timing delta does not extend the attacker's capabilities in that scenario.

**Status**: ✓ closed. Constant-time policy evaluation (eliminating the within-rule delta entirely) was originally a roadmap item, but the absolute-time analysis shows the residual leak is provably below remote-exploitability and irrelevant to the local-attacker threat model. The 1.6× ratio bound + the 5,000 ns absolute delta bound are pinned as regression invariants — a refactor that pushes either bound past its ceiling re-opens the question. Tracking: [`docs/internal/COVERAGE_HARDENING_PLAN.md △-1`](internal/COVERAGE_HARDENING_PLAN.md).
...
**v1.3 hardening (current)**:
- **Token issuance endpoint moved to admin port.** `POST /gvm/auth/token` now lives on the admin listener (`127.0.0.1:9090` by default, loopback-only unless `[server] allow_non_loopback_admin = true`), not the public agent port. An unauthenticated caller reaching the public proxy can no longer mint tokens for arbitrary `agent_id`s — token issuance is gated by operator shell access to the host (or by whatever auth gateway the operator fronts the admin port with).
- **Strict mode (`[jwt] strict = true`)**: when set, requests without a valid Bearer JWT *and* without a resolvable sandbox-peer mapping are rejected at the proxy with HTTP 401. Default `false` for backward compatibility; flipping it removes the fall-through to header-declared `X-GVM-Agent-Id` entirely. MITM path was already strict by default.
- **Revocation list (`[jwt] revocation_file = "<path>"`)**: a flat file listing `jti` claims to reject. Re-read on every verify (no proxy restart needed). Operators append via `POST /gvm/auth/revoke` on the admin port. Missing/unreadable file is fail-open (the revocation check is no-op) so a typo'd path doesn't lock all agents out — operators paranoid about availability should monitor the startup log line that prints the resolved path.
- **`token_id` (jti) bound into Merkle audit chain (event_hash v3, `gvm-event-v3:` prefix)**: every event records the authenticating JWT's `jti` (or `sandbox-peer:<sandbox_id>` for namespace-bound identity) and folds it into the SHA-256 leaf. An auditor can now prove not just *which agent* performed an action but *which token instance* authorised it — enabling cryptographic detection of token replay across the chain. Legacy v1/v2 WAL entries continue to verify (dispatcher chooses by field presence); new entries land as v3 once token_id is populated.

**v1.6 hardening (current)**:
- **HS256 (symmetric) signing REMOVED.** The v1.5 deprecation banner shipped first; this release completes the removal. `JwtAlgorithm` only has `Ed25519` now; `JwtKeyMaterial::Hmac` and `JwtSecret` are deleted. `[jwt] algorithm = "hs256"` errors at startup with a migration message. Operator upgrade path is in the v1.5 entry below and in `CHANGELOG.md` (2026-05-21 HS256-removal commit).
- **`[[jwt.keys]]` TOML table for multi-slot rotation**: the operator-facing config wiring (Phase D) for the slot rotation that landed in Phase B. Operators declare slots as a TOML array with `kid`, `seed_env`, `active`, `expires_at` per entry; the proxy reads each entry's env at load time and builds the slot vector. Existing single-key configs continue to work unchanged (`ed25519_seed_env` + `ed25519_key_id` fields stay supported as a degenerate single-slot path).

**v1.5 hardening**:
- **Admin port JWT middleware**: when `admin_listen` is non-loopback, the admin router is wrapped with `require_admin_jwt` (defense-in-depth: the bind-policy gate alone is not relied on). Requests without a Bearer token carrying `gvm_role = "admin"` are rejected 401 before reaching any handler. A one-shot bootstrap admin token is printed to stderr at first launch (kubeadm pattern). When the admin port is loopback (default), no middleware is installed — trust = OS shell access.
- **`gvm_role` claim**: added to JWT `Claims`. `None` for normal agent tokens, `Some("admin")` for operator tokens (synthetic `sub = "admin:<label>"`). NOT consulted by SRR rule matching — the v0.7 reframe deliberately removed cross-agent scope enforcement from rules; this is a narrow admin-port-only gate.
