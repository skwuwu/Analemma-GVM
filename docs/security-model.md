...
- **HS256 deprecation banner + `GVM_ENV=production` default = Ed25519**: when HS256 is the resolved algorithm, the proxy prints a prominent stderr banner + structured warning log explaining the symmetric-secret risk and pointing at the Ed25519 migration. When `GVM_ENV=production` is set and the operator did not explicitly choose an algorithm, the default flips from HS256 to Ed25519. HS256 will be removed in v1.0; the banner communicates the timeline explicitly. **No penalty latency**: artificial slowdowns are hostile UX and do not improve security — the deprecation is communicated, not punished.
