...
- **Symmetric HMAC**: the same `GVM_JWT_SECRET` signs and verifies. An external auditor cannot verify token authenticity without holding the issuer's secret. Asymmetric (Ed25519 / RS256) is a planned follow-up (the anchor-signing key infrastructure is already in place and would be reused).
