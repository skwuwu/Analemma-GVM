...
- **HS256 single-secret**: when `algorithm = "hs256"` (default), the same `GVM_JWT_SECRET` signs and verifies. Anyone with the secret can mint tokens. Operators wanting downstream-verifiable tokens or who don't want to distribute the signing key should switch to Ed25519.
- **Admin port unauthenticated when non-loopback**: setting `allow_non_loopback_admin = true` shifts the entire admin trust boundary onto the operator's fronting layer (mTLS terminator, VPN, IAP). The proxy itself does not authenticate admin callers — it relies on the bind-policy gate.
- **No key rotation primitive**: rotating an HS256 secret or an Ed25519 seed is a restart-with-new-env operation; tokens issued under the old key are immediately invalidated. There is no overlap window / multi-key validation. Operators wanting graceful rotation should rotate through revocation (issue new tokens, revoke old ones, then swap secrets after revocation list clears).
