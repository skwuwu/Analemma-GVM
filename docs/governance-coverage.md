...
| **Contained** | `gvm run --contained agent.py` | Docker bridge + host-side iptables egress lock (no MITM, no in-container HTTPS interception) | **EXPERIMENTAL — under construction** |

> **Contained mode (`--contained`) is EXPERIMENTAL.** Docker isolation primitives work (read-only FS, no-new-privileges, NET_ADMIN drop, resource limits, session UX, orphan cleanup). The host-side iptables egress lock (DOCKER-USER chain) is in place and catches non-cooperative clients (Node.js raw `https`, raw sockets) at the DROP rule. **NOT YET WIRED**: in-container DNAT to MITM, runtime CA injection into the container trust store, and `HTTPS_PROXY` env-strip — so transparent HTTPS interception inside the container does not work today. The flag is gated behind `cargo build --features contained` and is **not** present in the default release binary. For full HTTPS L7 inspection on Linux use `--sandbox`. (Source of truth: [crates/gvm-cli/src/main.rs:127-137](../crates/gvm-cli/src/main.rs#L127-L137).)
>
> **Platform scope**: iptables lives in the Docker host kernel. On native Linux and WSL2 (Windows), the host-side egress lock works; on native Windows and macOS (Docker Desktop's hidden VM is inaccessible to `iptables`), contained mode falls back to cooperative `HTTP_PROXY` routing with a visible warning, meaning non-cooperative clients can bypass. Run `gvm` from WSL2 on Windows for the host-side enforcement layer.
...
The agent runs in isolated Linux namespaces (user, PID, mount, network) with seccomp-BPF syscall filtering, TC ingress filter network enforcement (kernel-level, no userspace bypass), and overlayfs filesystem governance. This is the **production security boundary**.
