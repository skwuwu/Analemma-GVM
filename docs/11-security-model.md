...
| **WAL coverage gaps in proxy.rs** | Pre-classification failures (JWT 401, missing host 400, SRR lock poison 500) and proxy_handler rate limit (429) do not record to WAL. MITM path (tls_proxy.rs) is fully covered — Deny, RequireApproval, Throttle, classification error all record. proxy.rs cooperative/CONNECT paths have 7 gaps. | P1: Add WAL append to SRR lock poison, rate limit, shadow STRICT paths in proxy.rs. P2: Add minimal events for JWT/parse failures. Circuit breaker (503) is intentionally unrecorded (WAL already failing). |
