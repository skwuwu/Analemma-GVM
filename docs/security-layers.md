...
- **Time-bounded permission grants via IC-3 + orchestrator** are **planned for v0.7** (see roadmap in `docs/internal/CHANGELOG.md`). Rather than introducing in-runtime JWT-scope rule matching — which would promote JWT verification to a hot-path cross-agent privilege boundary — GVM exposes three primitives an external orchestrator composes: the existing IC-3 `RequireApproval` mechanism, a WAL event stream subscription, and a granular SRR rule add/remove API with `expires_at`. The runtime stays a recorder + enforcer; the orchestrator decides scope and duration outside the runtime where OS-process / namespace boundaries already provide structural enforcement. Same expressive power as per-agent JWT scoping without any cross-agent boundary inside the runtime.
